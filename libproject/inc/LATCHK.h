...
// and rotation-invariant LATCHK 512-bit binary
// feature descriptor as described in the 2015
// paper by Levi and Hassner:
//
// "LATCHK: Learned Arrangements of Three Patch Codes"
// http://arxiv.org/abs/1501.03719
//
// See also the ECCV 2016 Descriptor Workshop paper, of which I am a coauthor:
//
// "The CUDA LATCHK Binary Descriptor"
// http://arxiv.org/abs/1609.03986
//
// And the original LATCHK project's website:
// http://www.openu.ac.il/home/hassner/projects/LATCHK/
//
...
//
// All functionality is contained in the file LATCHK.h.
...
struct KeyPointK {
	...
	// angle in RADIANS
	KeyPointK(const float _x, const float _y, const float _scale, const float _angle) : x(_x), y(_y), scale(_scale), angle(_angle) {}
};
...
void _LATCHK(const int start, const int thread_stride, const uint8_t* const __restrict image, const int stride, std::vector<KeyPointK>& keypoints, uint64_t* const __restrict descriptors) {
	for (int i = start; i < start + thread_stride; ++i) {
		uint8_t* const __restrict desc = reinterpret_cast<uint8_t*>(descriptors + (i << 3));
		const KeyPointK pt = keypoints[i];
...
void LATCHK(const uint8_t* const __restrict image, const int width, const int height, const int stride, std::vector<KeyPointK>& keypoints, uint64_t* const __restrict descriptors) {
	keypoints.erase(std::remove_if(keypoints.begin(), keypoints.end(), [width, height](const KeyPointK& kp) {return kp.x <= 36 || kp.y <= 36 || kp.x >= width - 36 || kp.y >= height - 36; }), keypoints.end());
	...
	if (multithread) {
		...
		if (hw_concur > 1) {
			...
			for (; i < std::min(sz - 1, hw_concur - 1); ++i, start += thread_stride) {
				fut[i] = std::async(std::launch::async, _LATCHK, start, thread_stride, image, stride, std::ref(keypoints), descriptors);
			}
			fut[i] = std::async(std::launch::async, _LATCHK, start, sz - start, image, stride, std::ref(keypoints), descriptors);
			for (int j = 0; j <= i; ++j) fut[j].wait();
		}
		else {
			_LATCHK(0, sz, image, stride, std::ref(keypoints), descriptors);
		}
	}
	else {
		_LATCHK(0, sz, image, stride, std::ref(keypoints), descriptors);
